...
RSpec.describe PagySerializer do
  it "preserves the frontend prev key for a middle page" do
    result = described_class.one(Pagy::Offset.new(count: 50, page: 2, limit: 10)).stringify_keys
  ...
  it "returns null at pagination boundaries" do
    result = described_class.one(Pagy::Offset.new(count: 1, page: 1, limit: 10)).stringify_keys
