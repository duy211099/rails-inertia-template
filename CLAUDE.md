...
1. Rails controller calls `render inertia: 'ComponentName', props: { data: serializer.new(record).serializable_hash }`
2. `InertiaController` (base controller) shares `flash` and authenticated `user` props on every request
3. Serializers (in `app/serializers/`) use **Alba** and transform keys to **camelCase** for React
4. `typelizer` gem generates TypeScript types from serializers — run `npm run generate:types` after modifying serializers
...
See `.env.example` for Google OAuth credentials (`GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`) and production Cloudflare R2 settings. SQLite and the development email inbox need no external service credentials.

### Collection props and type safety

`ApplicationResource` includes Alba's Inertia helper. Collection controllers use
explicit page resources and `.to_inertia` for lazy prop evaluation on partial
reloads. Keep authorization and query scoping in controllers. Page prop names
stay as declared; model serializers use camelCase without transforming nested
audit JSON. `typelize_from` infers model fields and `typelize` declares computed
fields. Do not hand-edit generated types or duplicate Pagy types in the frontend.

### Development tooling

- Development email inbox: `/letter_opener` (never mounted in production).
- Production request summaries: Lograge JSON, without request parameters.
